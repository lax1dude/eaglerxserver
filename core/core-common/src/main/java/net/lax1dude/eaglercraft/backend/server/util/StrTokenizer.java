 ...
 * {@code ListIterator} interface. By default, it is set up like
...
public class StrTokenizer implements ListIterator<String>, Cloneable {
	 ...
	 * initializing it with the given input. The default for CSV processing will be
	 * trim whitespace from both ends (which can be overridden with the setTrimmer
	 * method).
	 * <p>
	...
	// -----------------------------------------------------------------------
	/**
	 * Constructs a tokenizer splitting on space, tab, newline and formfeed as per
	 * StringTokenizer, but with no text to tokenize.
	 * <p>
	...
	/**
	 * Constructs a tokenizer splitting using the specified delimiter matcher and
	 * handling quotes using the specified quote matcher.
	 *
	 * @param input the string which is to be parsed
	 * @param delim the field delimiter matcher
	...
	public StrTokenizer(final char[] input) {
		this.chars = new String(input).toCharArray();
	}
	...
	/**
	 * Gets the next token from the String. Equivalent to {@link #next()} except it
	 * returns null rather than throwing {@link NoSuchElementException} when no
	 * tokens remain.
	 *
	...
	/**
	 * Reset this tokenizer, giving it a new input string to parse. In this manner
	 * you can re-use a tokenizer with the same settings on multiple input lines.
	 *
	 ...
	 * you can re-use a tokenizer with the same settings on multiple input lines.
	 *
	 * @param input the new character array to tokenize, not cloned, null sets no
	 *              text to parse
	 * @return this, to enable chaining
	 */
	 ...
	 * called automatically by other (public) methods when required.
	 * <p>
	 * This method exists to allow subclasses to add code before or after the
	 * tokenization. For example, a subclass could alter the character array, offset
	 * or count to be parsed, or call the tokenizer multiple times on multiple
	 * strings. It is also be possible to filter the results.
	 * <p>
	 * {@code StrTokenizer} will always pass a zero offset and a count equal to the
	 * length of the array to this method, however a subclass may pass other values,
	 * or even an entirely different array.
	 *
	 * @param srcChars the character array being tokenized, may be null
	 * @param offset   the start position within the character array, must be valid
	 * @param count    the number of characters to tokenize, must be valid
	 * @return the modifiable list of String tokens, unmodifiable if null array or
	 *         zero count
	 */
	 ...
	 * @return the starting position of the next field (the character immediately
	 *         after the delimiter), or -1 if end of string found
	 */
	 ...
	 * @return the starting position of the next field (the character immediately
	 *         after the delimiter, or if end of string found, then the length of
	 *         string
	 */
	 ...
	 * delimiters to be entered as data. The default value is '"' (double quote).
	 *
	 ...
	 * <p>
	 * These characters are ignored when parsing the String, unless they are within
	 * a quoted region. The default value is not to ignore anything.
	 *
	...
	/**
	 * Sets whether the tokenizer should return empty tokens as null. The default
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets whether the tokenizer currently ignores empty tokens. The default for
	 ...
	 * it will be at the start of the token list. If a
