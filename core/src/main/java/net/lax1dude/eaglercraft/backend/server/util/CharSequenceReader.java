...
public class CharSequenceReader extends Reader implements Serializable {
	 ...
	 * <p>
	 * When de-serializing a CharSequenceReader that was serialized before this
	 * fields was added, this field will be initialized to 0, which gives the same
	 * behavior as before: start reading from the start.
	 * </p>
	 *
	 * @see #start()
	 * @since 2.7
	 ...
	 * <p>
	 * When de-serializing a CharSequenceReader that was serialized before this
	 * fields was added, this field will be initialized to {@code null}, which gives
	 * the same behavior as before: stop reading at the CharSequence's length. If
	 * this field was an int instead, it would be initialized to 0 when the
	 * CharSequenceReader is de-serialized, causing it to not return any characters
	 * at all.
	 * </p>
	 *
	 * @see #end()
	 ...
	 * character sequence. This allows the character sequence to grow or shrink in
	 * size without risking any {@link IndexOutOfBoundsException} to be thrown.
	 ...
	 * of the character sequence. This allows the character sequence to grow or
	 * shrink in size without risking any {@link IndexOutOfBoundsException} to be
	 * thrown. Instead, if the character sequence grows smaller than the start
	 * index, this instance will act as if all characters have been read; if the
	 * character sequence grows smaller than the end, this instance will use the
	 ...
	 * @throws IllegalArgumentException if the start index is negative, or if the
	...
	/**
	 * Returns the index in the character sequence to end reading at, taking into
	 * account its length.
	 *
	...
	private int end() {
		/*
		 * end == null for de-serialized instances that were serialized before start and
		 * end were added. Use Integer.MAX_VALUE to get the same behavior as before -
		 * use the entire CharSequence.
		 */
	 ...
	 *
	 * @return {@code true} if more characters from the character sequence are
	...
	/**
	 * Returns the index in the character sequence to start reading from, taking
