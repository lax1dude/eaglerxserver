...
final class RamUsageEstimator {
	...
	static {
		...
		if (JRE_IS_64BIT) {
			// Try to get compressed oops and object alignment (the default seems to be 8 on
			// Hotspot);
			// (this only works on 64 bit, on 32 bits the alignment and reference size is
			// fixed):
				...
				if (hotSpotBean != null) {
					...
					try {
						...
						compressedOops = Boolean
								.parseBoolean(vmOption.getClass().getMethod("getValue").invoke(vmOption).toString());
			...
			// array header is NUM_BYTES_OBJECT_HEADER + NUM_BYTES_INT, but aligned (object
			// alignment):
