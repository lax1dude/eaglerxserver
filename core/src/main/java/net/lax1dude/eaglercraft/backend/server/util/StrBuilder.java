...
public class StrBuilder implements CharSequence, Appendable, Serializable {
	 ...
	 *
	 * @param initialCapacity the initial capacity, zero or less will be converted
	 *                        to 32
	 */
	...
	/**
	 * Updates the length of the builder by either dropping the last characters or
	 * adding filler of Unicode zero.
	 *
	...
	/**
	 * Checks is the string builder is not empty (convenience Collections API style
	 * method).
	 * <p>
	 * This method is the same as checking {@link #length()} and is provided to
	 * match the API of Collections.
	 ...
	 * <p>
	 * This method does not reduce the size of the internal character buffer. To do
	 * that, call {@code clear()} followed by {@link #minimizeCapacity()}.
	 * <p>
	 * This method is the same as {@link #setLength(int)} called with zero and is
	 * provided to match the API of Collections.
	 *
	 * @return this, to enable chaining
	 */
	 ...
	 * @throws IndexOutOfBoundsException if startIndex is invalid, or if endIndex is
	 *                                   invalid (but endIndex greater than size is
	 *                                   valid)
	 */
	 ...
	 * This might be used to force the output to always use Unix line endings even
	 * when on Windows.
	 *
	 * @return this, to enable chaining
	 */
	...
	/**
	 * Appends part of a CharSequence to this string builder. Appending null will
	...
	/**
	 * Appends the contents of a char buffer to this string builder. Appending null
	...
	/**
	 * Appends part of a string buffer to this string builder. Appending null will
	 * call {@link #appendNull()}.
	 *
	 * @param str        the string to append
	 * @param startIndex the start index, inclusive, must be valid
	...
	/**
	 * Appends part of a StringBuilder to this string builder. Appending null will
	 * call {@link #appendNull()}.
	 *
	 * @param str        the StringBuilder to append
	 * @param startIndex the start index, inclusive, must be valid
	...
	/**
	 * Appends another string builder to this string builder. Appending null will
	 * call {@link #appendNull()}.
	 *
	...
	/**
	 * Appends part of a string builder to this string builder. Appending null will
	 * call {@link #appendNull()}.
	 *
	 * @param str        the string to append
	 * @param startIndex the start index, inclusive, must be valid
	...
	// -----------------------------------------------------------------------
	/**
	 * Appends an object followed by a new line to this string builder. Appending
	...
	/**
	 * Appends a char array followed by a new line to the string builder. Appending
	 * null will call {@link #appendNull()}.
	 *
	 * @param chars the char array to append
	 * @return this, to enable chaining
	 ...
	 * Appending a null iterable will have no effect. Each object is appended using
	 * {@link #append(Object)}.
	 *
	 ...
	 * Appending a null iterator will have no effect. Each object is appended using
	 * {@link #append(Object)}.
	 *
	...
	// -----------------------------------------------------------------------
	/**
	 * Appends an array placing separators between each value, but not before the
	 * first or after the last. Appending a null array will have no effect. Each
	...
	/**
	 * Appends an iterable placing separators between each value, but not before the
	 * first or after the last. Appending a null iterable will have no effect. Each
	 * object is appended using {@link #append(Object)}.
	 *
	 * @param iterable  the iterable to append
	 * @param separator the separator to use, null means no separator
	...
	/**
	 * Appends an iterator placing separators between each value, but not before the
	 * first or after the last. Appending a null iterator will have no effect. Each
	 * object is appended using {@link #append(Object)}.
	 *
	 * @param it        the iterator to append
	 * @param separator the separator to use, null means no separator
	...
	// -----------------------------------------------------------------------
	/**
	 * Appends a separator if the builder is currently non-empty. Appending a null
	 * separator will have no effect. The separator is appended using
	 * {@link #append(String)}.
	 * <p>
	...
	/**
	 * Appends one of both separators to the StrBuilder. If the builder is currently
	 * empty it will append the defaultIfEmpty-separator Otherwise it will append
	...
	/**
	 * Appends a separator if the builder is currently non-empty. The separator is
	...
	// -----------------------------------------------------------------------
	/**
	 * Appends an object to the builder padding on the left to a fixed width. The
	 * {@code toString} of the object is used. If the object is larger than the
	 * length, the left hand side is lost. If the object is null, the null text
	 * value is used.
	 *
	...
	/**
	 * Appends an object to the builder padding on the left to a fixed width. The
	 * {@code String.valueOf} of the {@code int} value is used. If the formatted
	...
	/**
	 * Appends an object to the builder padding on the right to a fixed length. The
	 * {@code toString} of the object is used. If the object is larger than the
	 * length, the right hand side is lost. If the object is null, null text value
	 * is used.
	 *
	 * @param obj     the object to append, null uses null text
	 * @param width   the fixed field width, zero or negative has no effect
	...
	/**
	 * Appends an object to the builder padding on the right to a fixed length. The
	 * {@code String.valueOf} of the {@code int} value is used. If the object is
	...
	// -----------------------------------------------------------------------
	/**
	 * Inserts the string representation of an object into this builder. Inserting
	...
	/**
	 * Inserts the string into this builder. Inserting null will use the stored null
	 * text value.
	 *
	 * @param index the index to add at, must be valid
	...
	/**
	 * Inserts the character array into this builder. Inserting null will use the
	...
	/**
	 * Inserts part of the character array into this builder. Inserting null will
	 * use the stored null text value.
	 *
	 * @param index  the index to add at, must be valid
	 * @param chars  the char array to insert
	 ...
	 * <p>
	 * Matchers can be used to perform advanced deletion behavior. For example you
	 * could write a matcher to delete all occurrences where the character 'a' is
	 ...
	 * <p>
	 * Matchers can be used to perform advanced deletion behavior. For example you
	 * could write a matcher to delete where the character 'a' is followed by a
	 * number.
	 *
	 * @param matcher the matcher to use to find the deletion, null causes no action
	 * @return this, to enable chaining
	...
	/**
	 * Replaces a portion of the string builder with another string. The length of
	 * the inserted string does not have to match the removed length.
	 *
	 * @param startIndex the start index, inclusive, must be valid
	 * @param endIndex   the end index, exclusive, must be valid except that if too
	 *                   large it is treated as end of string
	 ...
	 * <p>
	 * Matchers can be used to perform advanced replace behavior. For example you
	 * could write a matcher to replace all occurrences where the character 'a' is
	 * followed by a number.
	 *
	 ...
	 * <p>
	 * Matchers can be used to perform advanced replace behavior. For example you
	 * could write a matcher to replace where the character 'a' is followed by a
	 * number.
	 *
	 * @param matcher    the matcher to use to find the deletion, null causes no
	 *                   action
	 ...
	 * <p>
	 * Matchers can be used to perform advanced behavior. For example you could
	 * write a matcher to delete all occurrences where the character 'a' is followed
	 * by a number.
	 *
	 * @param matcher      the matcher to use to find the deletion, null causes no
	 *                     action
	...
	// -----------------------------------------------------------------------
	/**
	 * Trims the builder by removing characters less than or equal to a space from
	 ...
	 * Extracts a portion of this string builder as a string.
	 * <p>
	 * Note: This method treats an endIndex greater than the length of the builder
	 * as equal to the length of the builder, and continues without error, unlike
	...
	/**
	 * Extracts the leftmost characters from the string builder without throwing an
	 * exception.
	 * <p>
	 * This method extracts the left {@code length} characters from the builder. If
	 * this many characters are not available, the whole builder is returned. Thus
	...
	/**
	 * Extracts the rightmost characters from the string builder without throwing an
	 * exception.
	 * <p>
	 * This method extracts the right {@code length} characters from the builder. If
	 * this many characters are not available, the whole builder is returned. Thus
	 * the returned string may be shorter than the length requested.
	 *
	 * @param length the number of characters to extract, negative returns empty
	 ...
	 * <p>
	 * This method extracts {@code length} characters from the builder at the
	 * specified index. If the index is negative it is treated as zero. If the index
	 * is greater than the builder size, it is treated as the builder size. If the
	 * length is negative, the empty string is returned. If insufficient characters
	 * are available in the builder, as much as possible is returned. Thus the
	...
	/**
	 * Checks if the string builder contains a string matched using the specified
	 * matcher.
	 * <p>
	 * Matchers can be used to perform advanced searching behavior. For example you
	 * could write a matcher to search for the character 'a' followed by a number.
	 *
	 ...
	 * <p>
	 * Matchers can be used to perform advanced searching behavior. For example you
	 * could write a matcher to find the character 'a' followed by a number.
	 *
	 * @param matcher the matcher to use, null returns -1
	 ...
	 * <p>
	 * The returned tokenizer is linked to this builder. You may intermix calls to
	 * the builder and tokenizer within certain limits, however there is no
	 ...
	 * {@link StrTokenizer#reset(char[])} with a non-null value will break the link
	 * with the builder.
	 *
	 ...
	 * <p>
	 * This method allows the contents of the builder to be read using any standard
	 * method that expects a Reader.
	 * <p>
	 * To use, simply create a {@code StrBuilder}, populate it with data, call
	 * {@code asReader}, and then read away.
	 * <p>
	 * The internal character array is shared between the builder and the reader.
	 * This allows you to append to the builder after creating the reader, and the
	 * changes will be picked up. Note however, that no synchronization occurs, so
	 * you must perform all operations with the builder and the reader in one
	 * thread.
	 * <p>
	 ...
	 * populate away. The data is available at any time using the methods of the
	 * {@code StrBuilder}.
	 * <p>
	 * The internal character array is shared between the builder and the writer.
	 * This allows you to intermix calls that append to the builder and write using
	 * the writer and the changes will be occur correctly. Note however, that no
	 * synchronization occurs, so you must perform all operations with the builder
	 * and the writer in one thread.
	 * <p>
	...
	/**
	 * Checks the contents of this builder against another to see if they contain
	 ...
	 * <p>
	 * Note that unlike StringBuffer, the string version returned is independent of
	 * the string builder.
	 *
	...
	/**
	 * Gets a StringBuffer version of the string builder, creating a new instance
	...
	/**
	 * Gets a StringBuilder version of the string builder, creating a new instance
	 * each time the method is called.
	 *
